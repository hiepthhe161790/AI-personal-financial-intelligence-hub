    ...
    <div className="rounded-3xl bg-slate-950 dark:bg-slate-900/60 border border-slate-800 p-6 sm:p-8 space-y-6 shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Mở tài khoản đầu tư & thẻ tín dụng tối ưu chi phí từ các ngân hàng uy tín</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {AFFILIATE_OFFERS.map((offer) => (
          <div
            key={offer.id}
            className="p-5 rounded-2xl bg-slate-900/50 dark:bg-slate-950 border border-slate-800 hover:border-slate-600 transition-all flex flex-col justify-between space-y-4 group"
          >
            ...
            <a
              ...
              className="w-full py-2.5 rounded-xl bg-slate-950 dark:bg-slate-900 hover:bg-amber-500 hover:text-slate-950 text-amber-500 dark:text-amber-400 font-extrabold text-xs border border-amber-500/30 transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-md"
            >
